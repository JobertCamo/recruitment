...
@endphp
<div wire:key="{{ $job->id }}" class="space-y-3 lg:space-y-4 relative rounded-md py-4 px-4 h-64 md:h-40 lg:h-44 shadow-[rgba(0,_0,_0,_0.24)_0px_3px_8px] cursor-pointer transition duration-300 ease-in-out hover:ring-2 hover:ring-amber-500" @click="details = true" wire:click="resu('{{ $job->id }}')">
    ...
    <div class="text-wrap text-gray-500  w-auto lg:w-4/6">
        <p class="hidden md:block">{{ Str::limit($job->description, 220) }}</p>
        <p class="block md:hidden">{{ Str::limit($job->description, 150) }}</p>
    </div>
</div>